...
module Card_Suit :
  sig
    type t = Heart | Diamond | Club | Spade
    val t_of_sexp : Sexplib0.Sexp.t -> t
    val sexp_of_t : t -> Sexplib0.Sexp.t
    val compare : t -> t -> int
    val equal : t -> t -> bool
...
module Card :
  sig
    type t = { rank : Card_Rank.t; suit : Card_Suit.t; }
    val t_of_sexp : Sexplib0.Sexp.t -> t
    val sexp_of_t : t -> Sexplib0.Sexp.t
    val compare : t -> t -> int
    val equal : t -> t -> bool
...
module Player :
  sig
    ...
    val update_player_hand :
      t list -> id:Player_Idx.t -> new_hand:Card.t list -> t list
  end
module Move_error :
  sig
    type t =
...
module Group :
  sig
    ...
    val meets_requirement :
      rules:Rules.t ->
      current_req:t option -> t -> (unit, Move_error.t) Core._result
  end
module Play :
  sig
    type t = Play of Group.t | Pass
    val t_of_sexp : Sexplib0.Sexp.t -> t
    val sexp_of_t : t -> Sexplib0.Sexp.t
    val compare : t -> t -> int
    val equal : t -> t -> bool
...
module Phase :
  sig
    type t = Dealing | DeckPicking | Playing | RoundEnd
    val t_of_sexp : Sexplib0.Sexp.t -> t
    val sexp_of_t : t -> Sexplib0.Sexp.t
    val compare : t -> t -> int
    val equal : t -> t -> bool
...
module Game_State :
  sig
    ...
    val make_move :
      t -> Player.t -> Play.t -> (t, Move_error.t) Core.Result.t
  end
