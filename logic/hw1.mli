type player_id = int
type role = President | Citizen | Scum
type card_rank =
    Three
  | Four
  | Five
  ...
  | Ace
  | Two
type card_suit = Heart | Diamond | Club | Spade
type card = { rank : card_rank; suit : card_suit; }
type player = {
  id : player_id;
  name : string option;
  hand : card list;
...
type group = { rank : card_rank; count : int; cards : card list; }
type play = Play of group | Pass
type phase = Dealing | DeckPicking | Playing | RoundEnd
type turn_state = {
